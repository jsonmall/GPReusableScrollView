...
//    rtContent.size.width = rtContent.size.width * nBufferViews;
    rtContent.size.height = rtContent.size.height * nBufferViews;
...
-(void)layoutSubviews
{
    ...
    CGFloat offsetDiff = self.contentOffset.y - currentView.frame.origin.y;
    // 如果滑动幅度没有达到翻页，则不执行reusable策略
    if (fabs(offsetDiff) < self.frame.size.height) {
